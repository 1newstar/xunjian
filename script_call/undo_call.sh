...
echo ''>/tmp/xunjian/sqlfile/$i/undo_tablespace_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}script/undo_tablespace_useage.sh oracle@$i:/tmp/undo_tablespace_useage.sh

##call the remote server SQL script##
sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $i  'sh /tmp/undo_tablespace_useage.sh'>>/tmp/xunjian/sqlfile/$i/undo_tablespace_created_sql.sql

##execute sql##
sqlplus -S xunjian/oracle$TARGET_DATABASE @/tmp/xunjian/sqlfile/$i/undo_tablespace_created_sql.sql >>/tmp/xunjian/log/$i/undo_tablespace.log

echo ''
echo "sql script path is  /tmp/xunjian/sqlfile/$i/undo_tablespace_created_sql.sql"
echo "sql script execute log path is  /tmp/xunjian/log/$i/undo_tablespace.log"

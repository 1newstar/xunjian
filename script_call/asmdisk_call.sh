...
echo \######################################################################
echo \"asm_disk_useage\" process started at $LOCAL_HOST_TIME      
...
echo ''>/tmp/xunjian/sqlfile/$i/asmdisk_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}script/asm_disk_useage.sh oracle@$i:/tmp/asm_disk_useage.sh

##call the remote server SQL script##
sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no oracle@$i  'sh /tmp/asm_disk_useage.sh'>>/tmp/xunjian/sqlfile/$i/asmdisk_created_sql.sql

##execute sql##
sqlplus -S xunjian/oracle$TARGET_DATABASE @/tmp/xunjian/sqlfile/$i/asmdisk_created_sql.sql >>/tmp/xunjian/log/$i/asmdisk.log

echo ''
echo "The SQL script path is  /tmp/xunjian/sqlfile/"$i"/asmdisk_created_sql.sql"
echo "The SQL execute log path is  /tmp/xunjian/log/"$i"/asmdisk.log"

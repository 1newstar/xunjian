...
echo \######################################################################
echo \"redo_log_switch\" process started at $LOCAL_HOST_TIME      
...
echo ''>/tmp/xunjian/sqlfile/$i/redo_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}script/redo_log_switch.sh oracle@$i:/tmp/redo_log_switch.sh

##call the remote server SQL script##
sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $i  'sh /tmp/redo_log_switch.sh'>>/tmp/xunjian/sqlfile/$i/redo_created_sql.sql

##execute sql##
sqlplus -S xunjian/oracle$TARGET_DATABASE @/tmp/xunjian/sqlfile/$i/redo_created_sql.sql  >>/tmp/xunjian/log/$i/redo_log.log

echo ''
echo "sql script path is  /tmp/xunjian/sqlfile/"$i"/redo_created_sql.sql"
echo "sql script execute log path is  /tmp/xunjian/log/"$i"/redo_log.log"

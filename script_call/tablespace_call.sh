...
echo \######################################################################
echo \"tablespace_useage\" process started at $LOCAL_HOST_TIME      
...
sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $i  'sh /tmp/tablespace_useage.sh'>>/tmp/xunjian/sqlfile/$i/tablespace_created_sql.sql

##execute sql##
sqlplus -S xunjian/oracle$TARGET_DATABASE @/tmp/xunjian/sqlfile/$i/tablespace_created_sql.sql >>/tmp/xunjian/log/$i/tablespace.log

echo ''
echo "sql script path is  /tmp/xunjian/sqlfile/"$i"/tablespace_created_sql.sql"
echo "sql script execute log path is  /tmp/xunjian/log/"$i"/tablespace.log"

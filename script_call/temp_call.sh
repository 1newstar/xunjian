...
echo \######################################################################
echo \"temp_tablespace_useage\" process started at $LOCAL_HOST_TIME      
...
echo ''>/tmp/xunjian/sqlfile/$i/temp_tablespace_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}script/temp_tablespace_useage.sh oracle@$i:/tmp/temp_tablespace_useage.sh

##call the remote server SQL script##
sshpass -p $PASSWORD ssh -o StrictHostKeyChecking=no $i  'sh /tmp/temp_tablespace_useage.sh'>>/tmp/xunjian/sqlfile/$i/temp_tablespace_created_sql.sql

##execute sql##
sqlplus -S xunjian/oracle$TARGET_DATABASE @/tmp/xunjian/sqlfile/$i/temp_tablespace_created_sql.sql >>/tmp/xunjian/log/$i/temp_tablespace.log

echo ''
echo "sql script path is  /tmp/xunjian/sqlfile/$i/temp_tablespace_created_sql.sql"
echo "sql script execute log path is  /tmp/xunjian/log/$i/temp_tablespace.log"

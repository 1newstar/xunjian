...
echo ''>/tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/memory_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}/script/memory_useage.sh oracle@$ip:/tmp/memory_useage.sh
...
echo "sql script execute log path is  /tmp/xunjian/log/"$SOURCE_DB_HOSTNAME"/memo.log"

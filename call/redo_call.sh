...
cat /home/oracle/xunjian/db_info/target_db.cnf|grep target_db_info|awk '{print $2}'|awk -F : '{print $2}'
...
echo ''>/tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/redo_created_sql.sql


##connect source database and get database data##
sqlplus -S $XUNJIAN_USER/$XUNJIAN_PASSWORD@$ip:$SOURCE_DB_PORT/$SOURCE_DB_INSTANCE  @$INSTALL_PATH/script/redo_log_switch.sql >>/tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/redo_created_sql.sql


##execute sql##
sqlplus -S $XUNJIAN_USER/$XUNJIAN_PASSWORD@$TARGET_DB_IP:$TARGET_DB_PORT/$TARGET_DB_INSTANCE @/tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/redo_created_sql.sql >>/tmp/xunjian/log/$SOURCE_DB_HOSTNAME/redo_log.log




echo ''
echo "The SQL script path is  /tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/redo_created_sql.sql"
echo "The SQL execute log path is  /tmp/xunjian/log/$SOURCE_DB_HOSTNAME/redo_log.log"

...
echo ''>/tmp/xunjian/sqlfile/$SOURCE_DB_HOSTNAME/disk_created_sql.sql
...
sshpass -p $PASSWORD scp -o StrictHostKeyChecking=no ${INSTALL_PATH}/script/disk_useage.sh oracle@$ip:/tmp/disk_useage.sh
...
echo "The SQL execute log path is  /tmp/xunjian/log/$SOURCE_DB_HOSTNAME/disk.log"
